...
# --------------------------------
# Programske skupine - zaključene
# --------------------------------


colnames(tabela_programi_zakljuceni) <- c(
  "Šifra",
  "Naziv",
  "Vodja",
  "Začetek",
...
# Dodaj prazen stolpec za številke
kc_prog_num <- tabela_programi_zakljuceni %>%
  mutate(` ` = "", .before = 1)    # Prazen stolpec
...
kc_prog_filter <- kc_prog_num %>%
  arrange(desc(`Začetek`)) %>%
  mutate(
    filter_org = if_else(`Vodilna org` == "UKC", "UKCL", "Drugo")
  )

# SharedData objekt
shared_prog <- SharedData$new(kc_prog_filter)
...
tbl_09 <- reactable(
  ...
  columns = list(
    ...
    Naziv = colDef(align = "left", minWidth = 150, sortable = FALSE),
    Vodja = colDef(align = "left", minWidth = 80, sortable = FALSE),
    `Vodilna org` = colDef(align = "left", minWidth = 80),
    `Začetek` = colDef(cell = function(value) {
      format(value, "%b %Y")
    }),
