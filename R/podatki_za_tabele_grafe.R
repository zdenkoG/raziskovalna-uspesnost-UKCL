...
## ZAKLJUČENI PROGRAMI
tabela_programi_zakljuceni <- 
  prog %>% 
  select(code, name, vodja = raz.x, zacetek, konec, st_clan, st_raz = raz.y, fte, org_sifra, org) %>% 
  as_tibble() %>% 
  filter(konec < Sys.Date()) %>% 
  mutate(
    org_st = str_count(org_sifra, ",") + 1,
    org_ime = str_extract(org, "^[^,]+"),
    org_ime = if_else(org_ime == "NA", "ZTM", org_ime),
    zacetek = as.Date(zacetek),
    konec = as.Date(konec)) %>% 
  select(-c(org, org_sifra)) 
